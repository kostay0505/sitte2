...
/**
 * Выгрузка в Google Sheets (ТЗ №4 Ч4.4). Логика 1:1 со старым parsed-base/GoogleSheetsService
 * (та же таблица/лист/колонки B:E — другая группа привыкла к формату). Вынесено в отдельный
 * @Global-модуль, чтобы пережить вывод ParsedBase (Ч3).
 * Формат строки: B=бренд, C=описание, D=цена (с валютой), E=url.
 */
@Injectable()
