...
export interface UsedfullProduct {
    id: string;
    externalId: string;
    url: string;
    title: string;
...
@Injectable()
export class UsedfullRepository {
    constructor(
        @Inject('DATABASE') private readonly db: Database,
    ) { }

    async findAll(status: 'active' | 'archived', page: number, limit: number, isNew?: boolean, notInBase?: boolean): Promise<{ items: UsedfullProduct[]; total: number }> {
        const offset = (page - 1) * limit;
        const extra = (isNew ? ' AND isNew = 1' : '') + (notInBase ? ' AND baseItemId IS NULL' : '');

        const countRes = await this.db.execute(sql`
            SELECT COUNT(*) as total FROM UsedfullProducts WHERE status = ${status}${sql.raw(extra)}
        `) as unknown as SqlQueryResult<{ total: number }>;
        const total = Number(((countRes[0]) as unknown as any[])[0]?.total ?? 0);

        const result = await this.db.execute(sql`
            SELECT * FROM UsedfullProducts WHERE status = ${status}${sql.raw(extra)}
    ...
    async findById(id: string): Promise<UsedfullProduct | null> {
        const result = await this.db.execute(sql`
            SELECT * FROM UsedfullProducts WHERE id = ${id} LIMIT 1
        `) as unknown as SqlQueryResult<any>;
    ...
    async findByExternalId(externalId: string): Promise<UsedfullProduct | null> {
        const result = await this.db.execute(sql`
            SELECT id FROM UsedfullProducts WHERE externalId = ${externalId} LIMIT 1
        `) as unknown as SqlQueryResult<any>;
        if (!Array.isArray(result[0])) return null;
        const row = (result[0] as any[])[0];
        return row ? this.mapRow(row) : null;
    ...
    async findActiveExternalIds(): Promise<string[]> {
        const result = await this.db.execute(sql`
            SELECT externalId FROM UsedfullProducts WHERE status = 'active'
        `) as unknown as SqlQueryResult<any>;
    ...
    async setStatus(id: string, status: 'active' | 'archived' | 'blacklisted'): Promise<void> {
        await this.db.execute(sql`
            UPDATE UsedfullProducts SET status = ${status}, updatedAt = NOW() WHERE id = ${id}
        `);
        if (status === 'blacklisted') {
            await this.db.execute(sql`UPDATE ParsedBase SET status = 'blacklisted', updatedAt = NOW() WHERE source = 'usedfull' AND sourceId = ${id} AND productId IS NOT NULL`);
            await this.db.execute(sql`DELETE FROM ParsedBase WHERE source = 'usedfull' AND sourceId = ${id} AND productId IS NULL`);
        }
    }
    ...
    async clearNew(id: string): Promise<void> {
        await this.db.execute(sql`
            UPDATE UsedfullProducts SET isNew = 0, updatedAt = NOW() WHERE id = ${id} AND isNew = 1
        `);
    }
    ...
    private mapRow(row: any): UsedfullProduct {
        return {
            ...row,
            images: row.images
