...
export interface ArriusProduct {
    id: string;
    externalId: number;
    url: string;
    title: string;
...
@Injectable()
export class ArriusRepository {
    constructor(
        @Inject('DATABASE') private readonly db: Database,
    ) { }

    async findAll(status: 'active' | 'archived', page: number, limit: number, isNew?: boolean, notInBase?: boolean): Promise<{ items: ArriusProduct[]; total: number }> {
        const offset = (page - 1) * limit;
        const extra = (isNew ? ' AND isNew = 1' : '') + (notInBase ? ' AND baseItemId IS NULL' : '');

        const countRes = await this.db.execute(sql`
            SELECT COUNT(*) as total FROM ArriusProducts WHERE status = ${status}${sql.raw(extra)}
        `) as unknown as SqlQueryResult<{ total: number }>;
        const total = Number(((countRes[0]) as unknown as any[])[0]?.total ?? 0);

        const result = await this.db.execute(sql`
            SELECT * FROM ArriusProducts WHERE status = ${status}${sql.raw(extra)}
    ...
    async findById(id: string): Promise<ArriusProduct | null> {
        const result = await this.db.execute(sql`
            SELECT * FROM ArriusProducts WHERE id = ${id} LIMIT 1
        `) as unknown as SqlQueryResult<any>;
    ...
    async findByExternalId(externalId: number): Promise<ArriusProduct | null> {
        const result = await this.db.execute(sql`
            SELECT id FROM ArriusProducts WHERE externalId = ${externalId} LIMIT 1
        `) as unknown as SqlQueryResult<any>;
        if (!Array.isArray(result[0])) return null;
        const row = (result[0] as any[])[0];
        return row ? this.mapRow(row) : null;
    ...
    async findActiveExternalIds(): Promise<number[]> {
        const result = await this.db.execute(sql`
            SELECT externalId FROM ArriusProducts WHERE status = 'active'
        `) as unknown as SqlQueryResult<any>;
    ...
    async create(data: {
        ...
        await this.db.execute(sql`
            INSERT INTO ArriusProducts
                (id, externalId, url, title, description, price, images, categories, brand, sku,
                 status, siteStatus, isNew, imagesDownloaded, publishedAt, lastCheckedAt, createdAt, updatedAt)
            VALUES (
                ${id},
                ${data.externalId},
                ${data.url},
    ...
    async setStatus(id: string, status: 'active' | 'archived' | 'blacklisted'): Promise<void> {
        await this.db.execute(sql`
            UPDATE ArriusProducts SET status = ${status}, updatedAt = NOW() WHERE id = ${id}
        `);
        if (status === 'blacklisted') {
            await this.db.execute(sql`UPDATE ParsedBase SET status = 'blacklisted', updatedAt = NOW() WHERE source = 'arrius' AND sourceId = ${id} AND productId IS NOT NULL`);
            await this.db.execute(sql`DELETE FROM ParsedBase WHERE source = 'arrius' AND sourceId = ${id} AND productId IS NULL`);
        }
    }

    // Mark products no longer on site — auto-archive
    ...
    // Restore siteStatus for products that reappear
    ...
    async clearNew(id: string): Promise<void> {
        await this.db.execute(sql`
            UPDATE ArriusProducts SET isNew = 0, updatedAt = NOW() WHERE id = ${id} AND isNew = 1
        `);
    }
    ...
    private mapRow(row: any): ArriusProduct {
        return {
            ...row,
