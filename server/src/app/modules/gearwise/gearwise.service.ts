...
import { GearwiseRepository } from './gearwise.repository';
...
const USER_PAGE_URL = 'https://used-stage-equipment.com/?modul=ads&site=userads&userid=26733';
const SITE_BASE = 'https://used-stage-equipment.com';
const CDN_BASE = 'https://cdn.g-vt.de/use/';
...
/**
 * Parse the user listings page: extract all ad IDs, titles, prices, thumbnail images.
 * List HTML pattern:
 *   <a href='ad-{ID}-{slug}' class='hdl_link'>Title</a>
 *   <div class='index-item-price-pro'>368€ VB</div>
 *   <img class='product-image' src='https://cdn.g-vt.de/use/thumbnail%2F{filename}'>
 */
function parseListingPage(html: string): Array<{ externalId: string; slug: string; title: string; price: string | null; thumbImage: string | null }> {
    ...
    // Find each listing block by the hdl_link anchor
    ...
    while ((m = linkRe.exec(html)) !== null) {
        ...
        // Look for thumbnail image near this match
...
/**
 * Convert thumbnail CDN URL to original-size URL.
 * thumbnail URL: https://cdn.g-vt.de/use/thumbnail%2F{filename}
 * original URL:  https://cdn.g-vt.de/use/{filename}
 */
...
/**
 * Parse the detail page of a listing to get:
 * - Full gallery images
...
function parseDetailPage(html: string, fallbackThumb: string | null): { images: string[]; category: string | null; publishedAt: string | null; description: string | null } {
    // Extract gallery images — USE3 uses a lightbox pattern with full-size links
    const images: string[] = [];

    // Pattern 1: <a href="https://cdn.g-vt.de/use/{filename}"> wrapping img
    ...
    // Pattern 2: img src pointing to non-thumbnail CDN images
    if (images.length === 0) {
    ...
    // Fallback: use thumbnail converted to original
    ...
    // Pattern: <a ...>Category Name</a> in breadcrumb area
    ...
    if (breadcrumbM) {
        ...
        // Skip first (Home) and take second-to-last as category if available
        if (links.length >= 2) {
...
@Injectable()
export class GearwiseService {
    private readonly logger = new Logger(GearwiseService.name);

    constructor(
        private readonly parserSettings: ParserSettingsService,
        private readonly repository: GearwiseRepository,
    ...
    async runParser(): Promise<void> {
        ...
        if (!(await this.parserSettings.isEnabled('gearwise'))) return;
        this.logger.log('Gearwise parser: starting');
        try {
            await this.parseAndSave();
        } catch (e: any) {
            this.logger.error('Gearwise parser error: ' + e.message);
        }
    }
    ...
    async parseAndSave(): Promise<{ added: number; errors: number }> {
        ...
        try {
            ...
            this.logger.log(`Gearwise: found ${listings.length} listings on user page`);

            const siteIds = listings.map(l => l.externalId);

            const notFoundCount = await this.repository.markNotFound(siteIds);
            if (notFoundCount > 0) this.logger.log(`Gearwise: marked ${notFoundCount} as not_found`);
            ...
            for (const listing of listings) {
                try {
                    ...
                    try {
                        ...
                        this.logger.debug(`Gearwise: could not fetch detail ${url}: ${e.message}`);
                        // Use thumbnail as fallback image
                    ...
                    this.logger.warn(`Gearwise: failed item ${listing?.externalId}: ${e.message}`);
                    errors++;
                }
            }
        } catch (e: any) {
            this.logger.error('Gearwise fetch error: ' + e.message);
            errors++;
        }

        this.logger.log(`Gearwise parser done: +${added} new, ${errors} errors`);
        await this.parserNotification.notify('Gearwise', 'used-stage-equipment.com', newItems);
        return { added, errors };
    }
    ...
    async downloadPhotos(id: string): Promise<{ success: boolean; count: number; dir: string; driveId?: string }> {
        ...
        for (let i = 0; i < product.images.length; i++) {
            ...
            try {
                ...
                this.logger.warn(`Gearwise download: failed ${imgUrl}: ${e.message}`);
            }
        }
        ...
        try {
            ...
            const sourceFolderId = await this.ensureFolder('Gearwise', parserFolderId);
            ...
            for (const localPath of localPaths) {
                try {
                    ...
                    this.logger.warn(`Gearwise Drive register failed: ${e.message}`);
                }
            }
        } catch (e: any) {
            this.logger.warn(`Gearwise Drive registration error: ${e.message}`);
        }
